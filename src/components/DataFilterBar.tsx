          ...
          {filterFields.map(field => (
            <div key={field.id} className="space-y-1.5">
              ...
              <div className="relative">
                <select
                  value={activeFilters[field.id] || 'all'}
                  onChange={e => onFilterChange?.(field.id, e.target.value)}
                  className="w-full py-2 px-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/50 dark:text-white appearance-none cursor-pointer pr-8"
                >
